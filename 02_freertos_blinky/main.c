...
#define RED_LED      NRF_GPIO_PIN_MAP(1, 15) // 红灯：Port 1, Pin 15
#define GREEN_LED    NRF_GPIO_PIN_MAP(1, 13) // 绿灯：Port 1, Pin 13

SemaphoreHandle_t x_key = NULL; //声明一个全局变量xkey，它的类型是 SemaphoreHandle_t（信号量句柄）。它在程序中代表了那把唯一的“钥匙”，因为需要在红灯和绿灯两个不同的函数里去申请它，所以必须定义为全局变量。
static void red_led_task(void *pv_parameters) {
  (void)pv_parameters;
  nrf_gpio_cfg_output(RED_LED); //设置为输出模式
  nrf_gpio_pin_clear(RED_LED); //初始化，初始为高电平，LED灯熄灭
  for (;;) { //无限循环
    if (xSemaphoreTake(x_key, portMAX_DELAY) == pdTRUE) {
      nrf_gpio_pin_set(RED_LED); //LED灯亮
      vTaskDelay(pdMS_TO_TICKS(500));
      nrf_gpio_pin_clear(RED_LED);
      xSemaphoreGive(x_key);
    }
    vTaskDelay(pdMS_TO_TICKS(50));
  }
}

static void green_led_task(void *pv_parameters) {
  (void)pv_parameters;
...
int main(void) {
  ...
  x_key = xSemaphoreCreateMutex();
  if (x_key != NULL) {
    xTaskCreate(red_led_task, "red", configMINIMAL_STACK_SIZE, NULL, 1, NULL);
    xTaskCreate(green_led_task, "green", configMINIMAL_STACK_SIZE, NULL, 1, NULL);
    vTaskStartScheduler();
  }
  while (true) { }
}