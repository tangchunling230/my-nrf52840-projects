...
SemaphoreHandle_t key = NULL; // 声明一个全局变量xkey，它的类型是 SemaphoreHandle_t（信号量句柄）。它在程序中代表了那把唯一的“钥匙”，因为需要在红灯和绿灯两个不同的函数里去申请它，所以必须定义为全局变量。
static void red_led_task(void *parameters) {
  (void)parameters;
...
static void green_led_task(void *parameters) {
  (void)parameters;
...
int cpu_set_vcore(uint32_t voltage) {
...
int main(void) {
  cpu_set_vcore(3000); // 配置VCORE电压的函数，voltage的单位是 mV
